...
function get_proportional_coordinates(cropper_instance, x1_func, y1_func, x2_func, y2_func) {
    ...
    A summary of the process is in Figure \ref{fig:roiprocessing}. Let $\pmb{A}$, $\pmb{B}$ be an $n \times 4$, matrix where $\pmb{A}_{ij}$, $\pmb{B}_{ij} \in \mathbb{R}^{n \times 4}$, $n \in \mathbb{N}$ is the number of frames, ${\pmb{A}}_{n1} \equiv x_1$, ${\pmb{A}}_{n2} \equiv y_1$, ${\pmb{A}}_{n3} \equiv x_2$, ${\pmb{A}}_{n4} \equiv y_2$, and the same for $\pmb{B}$. $T$ is the threshold.
